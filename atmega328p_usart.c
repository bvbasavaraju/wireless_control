...
/*
 * @name   	USARTRegInit(USART_StructureType)
 * @brief	This function is to update the Golbal variables with proper Registers!
 * @param  	USARTConfig   - Structure to configure the USARTx
 * @retval	None
 */
void USARTRegInit(USART_StructureType USARTConfig)
{
    RegA = &(UCSR0A);
    RegB = &(UCSR0B);
    RegC = &(UCSR0C);
...
USARTRX_IRQHandler()
{
	//static uint8_t countCR = 0;
	//static uint8_t countLF = 0;
	uint16_t ch;
	ch = USART_GetChar();

    /*if(countCR != 2)
	{
	    gGSM_Response[gIndex] = ch;
        gIndex++;
        if(ch == '\n')
            countCR++;
	}
    ...
    {
        gGSM_Response[gIndex] = '\0';
        countCR = 0;
        gReceive_Buffer_Full = 1;
    }*/

	if(ch != '\n' && (gIndex < (BUFFER_LENGTH - 1)))		// LF - New line Feed character will be received only once!
	{
		if(ch != '\r')
		{
			gGSM_Response[gIndex] = ch;
	...
	{
		gReceive_Buffer_Full = 1;
		gGSM_Response[gIndex] = '\0';	//For GSM Module
	}*/
}
